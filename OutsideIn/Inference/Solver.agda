...
module OutsideIn.Inference.Solver(x : X) where  
  ...
  solver : {x : Set}{s : Shape} → (n : ℕ) → AxiomScheme →  QConstraint (x ⨁ n) →  SeparatedConstraint (x ⨁ n) s
  ...
  ... | Unsolved {m = m} Qr θ = if solveImps axioms (conjConstraint Qr (qc-substitute θ given)) (substituteImp θ imps) 
                                 then suc (Unsolved {m = m} Qr θ) 
                                 else zero
  solveImps axioms given (imp-ε) = true
  solveImps axioms given (imp (∃ n · Q ⊃ C)) with solver n axioms (conjConstraint (QC-f.map (PlusN-m.unit {n}) given ) Q)  C
  ... | suc (Solved θ) = true
  ... | suc (Unsolved Qr θ) = false
  ... | zero = false
  solveImps axioms given (a imp-∧ b) = solveImps axioms given a ∧ solveImps axioms given b 